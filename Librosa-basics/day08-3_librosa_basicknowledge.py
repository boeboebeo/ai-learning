    ...
    ex. 오디오 길이 y = 96256 samples (sr = 48000), hop_length = 512
        ...
        프레임 수 = 96256 / 512 = 188 프레임
            ...
            ** d = RMS 프레임 간의 시간 간격. = hop / sr = 512 / 48000 = 0.010667
            => rms 는 아래와 같은 시간 간격으로 각각의 프레임으로 출력됨 (rms 는 0.0106초 마다 1개씩 나옴)
                (오디오를 512 샘플씩 건너뛰며 계산)
...
**rms 는 실수 신호의 절반 + 1만 반환: (주파수가 대칭이므로, 양의 주파수만 저장 (Nyquist 정리))
    ...
    따라서 위의 188개의 프레임을 가지고 있는 rms 배열로 rms_fft 를 하면 188/2 + 1 한 95개의 len이 나오게 되고,
    그에 1:1 대응되는 rms_freqs 가 출력됨 (각 프)
